...
for tryindex = 1:10
    ...
    load(savefile);
%     savefile=sprintf('CommonCC1-Node_%d_density_%d_SV_%d_try_%d_re_%d',T,N*100,SV,tryindex,re);
%     load(savefile);
    [data(re,tryindex),out(re,tryindex),model(re,tryindex),SS(re,tryindex)] = measureFCM(A,y,xp_MA,SV,T,K,W,AA,yy);
%     [data1(tryindex),out1(tryindex),model1(tryindex),SS1(tryindex)] = measureFCM(A,y,xp_MA1,SV,T,K,W,AA,yy);
end
end
errorbar(mean(data,2),std(data'),'r-o')
